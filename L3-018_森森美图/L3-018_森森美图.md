...
def entry
  main = lambda do ||
    ...
    p = 2
    score = py_iterable(py_range(n)).flat_map { |i|; [py_slice(a, (p + (i * m)), (p + ((i + 1) * m)), nil)] }
    p += (n * m)
      ...
      while py_truth(pq)
        ...
        for dx in [(-1), 0, 1]
          for dy in [(-1), 0, 1]
            ...
            nx, ny = [(x + dx), (y + dy)]
            if (!py_truth((((0 <= nx) && (nx < m)) && ((0 <= ny) && (ny < n)))))
              next
            end
            ...
            extra = ((py_truth(dx) && py_truth(dy)) ? ((score[y][x] + score[ny][nx]) * (Math.sqrt(2) - 1)) : 0)
            nd = ((du + score[ny][nx]) + extra)
            if ((nd < d[ny][nx]))
              d[ny][nx] = nd
              pq.push([nd, nx, ny]); pq.sort!
            end
          end
        end
      end
    ...
    ans = ((((x >= py_div(inf, 2))) || ((y >= py_div(inf, 2)))) ? py_min(x, y) : (((x + y) - score[sy][sx]) - score[ey][ex]))
