...
(1..n).each { |u| (1..n).each { |w| masks[u] |= 1 << (w - 1) if descendant.call(w, u) } }
full = (1 << n) - 1
memo = {}
solve =
  lambda do |mask, day|
    ...
    if mask == full
      ...
      (1..n).each do |u|
        ...
        answer =
          (
            answer +
              mod_pow(cost[day], masks[u].digits(2).sum, mod) * solve.call(mask | masks[u], day + 1)
          ) % mod
      end
      memo[key] = answer
    end
  ...
  lambda do |mask, day|
    ...
    if mask == full
      ...
      (1..n).each do |u|
        ...
        answer =
          (answer + mod_pow.call(cost[day], bits, mod) * solve.call(mask | masks[u], day + 1)) % mod
      end
      memo[key] = answer
    end
  end
