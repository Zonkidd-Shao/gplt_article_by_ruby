...
normalize = lambda { |num, den| den.negative? ? [-num, -den] : [num, den] }
...
100.times do
  ...
  if dx.positive?
    ...
    upper_bound = bound if upper_bound.nil? || compare.call(bound, upper_bound).negative?
  elsif dx.negative?
    bound = normalize.call(high[q] - low[p], dx)
    lower_bound = bound if lower_bound.nil? || compare.call(bound, lower_bound).positive?
  end
