...
def entry
  ...
  last = {}
  out = []
  valid = lambda { |s| return(((py_len(s) == 18)) && py_truth((s.to_s.match?(/\A[0-9]+\z/)))) }
  for day in py_range(1, (py_d + 1))
    ...
    for _, _, name, id_, _ in py_sorted(r)
      if (
           ((py_len(out) >= 0)) && py_truth(py_s) && py_truth(valid.call(id_)) &&
             ((!py_in(id_, last)) || ((day > (last[id_] + py_p)))) && ((py_s > 0))
         )
        out.append(((name + " ") + id_))
        last[id_] = day
        py_s -= 1
