...
# 题目：判断素数
# 实现原理：
#   只需试除不超过平方根的整数；若没有因子则判定为素数。
# 处理流程：
#   读取测试数据 → 对每个数试除 → 输出素数判断结果。
#
  ...
  prime =
    lambda do |x|
      return(
        ((x >= 2)) &&
          py_truth(
            py_iterable(py_range(2, ((Math.sqrt(x).to_i) + 1))).flat_map { |d| [(x % d)] }.all?
          )
      )
    end
  py_print(
    py_iterable(py_range(n)).flat_map { |_| [(prime.call(py_next(it)) ? "Yes" : "No")] }.join("\n"),
    sep: " ",
    ending: "\n"
  )
end
