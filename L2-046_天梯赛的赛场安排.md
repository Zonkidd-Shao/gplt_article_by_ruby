...
完整实现位于同目录的 [main.rb](./L2-046_天梯赛的赛场安排/main.rb)，以下代码与该入口文件保持一致。
...
def entry
  ...
  heap = py_iterable(py_range(n)).flat_map { |i|; next [] unless py_truth(rem[i]); [[(-rem[i]), i]] }
  heap.sort!
  contact = ([0] * n)
  rooms = []
  while py_truth(heap)
    ...
    if ((x >= py_c))
      ...
      contact[i] += 1
        ...
        cand = py_iterable(py_enumerate(rooms)).flat_map { |__item_0|; j, r = __item_0; next [] unless py_truth(((r >= x))); [[r, j]] }
        if py_truth(cand)
          r, j = py_min(cand)
          rooms[j] -= x
          else
    ...
    if py_truth(rem[i])
      heap.push([(-rem[i]), i]); heap.sort!
    end
  end
  py_print([*py_iterable(py_range(n)).flat_map { |i|; ["%s %s" % [names[i], contact[i]]] }, py_str(py_len(rooms))].join("\n"), sep: " ", ending: "\n")
end
entry
```
